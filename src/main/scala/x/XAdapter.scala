...
object XAdapter {
  def act(conn: X)(action: ConnectionAction): Unit = action match {
    ...
    case Close(window) =>
      ...
      conn.flush()
    case MouseMoveStart(window) =>
      println("movestart")
      println(window)
      conn.warpPointerForMove(window)
      conn.grabPointer(window)
      conn.flush()
    case MouseResizeStart(window) =>
      println("resizestart")
      println(window)
      conn.warpPointerForResize(window)
      conn.grabPointer(window)
      conn.flush()
    case MouseMoving(window) =>
      println("moving")
      println(window)
      conn.moveToPointer(window)
      conn.flush()
    case MouseResizing(window) =>
      println("resizing")
      println(window)
      conn.resizeToPointer(window)
      conn.flush()
    case MouseMoveEnd(window) =>
      println("moveend")
      println(window)
      conn.ungrabPointer()
      conn.flush()
    case MouseResizeEnd(window) =>
      println("resizeend")
      println(window)
      conn.ungrabPointer()
      conn.flush()
